...
const signUp = async (req, res) => {
    try {
        const { first_name, last_name, gender, phone_number, nin_number, email, password, address, account_type = "personal", // 'personal' or 'business'
        // Business-specific fields
        business_name, business_type, CAC_number, business_location, } = req.body;
        // Common validation
        if (!first_name ||
            !last_name ||
            !gender ||
        ...
        const existingEmailResult = await database_1.default.query("SELECT user_id FROM users WHERE user_email = $1", [email]);
        ...
        if (account_type === "personal" && nin_number) {
            const existingNINResult = await database_1.default.query("SELECT user_id FROM users WHERE nin_number = $1", [nin_number]);
        ...
        if (account_type === "business") {
            ...
            const existingBusinessResult = await database_1.default.query("SELECT user_id FROM users WHERE CAC_number = $1", [CAC_number]);
        ...
        `
          ...
          <p>Thank you for your patience.</p>
        `;
            await (0, emailConfig_1.sendEmail)(email, emailSubject, emailTemplate);
            res.status(201).json({
                success: true,
...
const getPendingApprovals = async (_req, res) => {
    try {
        const pendingUsersResult = await database_1.default.query(`SELECT u.user_id, u.user_name, u.user_firstname, u.user_lastname, u.user_phone, u.nin_number, 
...
const approveAccount = async (req, res) => {
    try {
       ...
       WHERE user_id = $3 AND account_status = 'pending'`, [activationCode, activationExpires, user_id]);
        ...
        const usersResult = await database_1.default.query("SELECT user_email, user_firstname, user_lastname, account_type, business_name FROM users WHERE user_id = $1", [user_id]);
      ...
      `
        ...
        <p>Welcome to our platform!</p>
      `;
        await (0, emailConfig_1.sendEmail)(user.user_email, emailSubject, emailTemplate);
        res.json({
            success: true,
...
const rejectAccount = async (req, res) => {
    try {
       ...
       WHERE user_id = $1 AND account_status = 'pending'`, [user_id]);
        if (result.rowCount === 0) {
            res.status(404).json({
                error: true,
                message: "User not found or already processed",
        ...
        await (0, emailConfig_1.sendEmail)(user.user_email, "Account Registration Update", `
...
exports.rejectAccount = rejectAccount;
/**
...
const signIn = async (req, res) => {
    try {
        ...
        // Find user
        const usersResult = await database_1.default.query(`SELECT user_id, user_email, user_password, user_firstname, user_lastname, 
       ...
       FROM users WHERE user_email = $1`, [email]);
        ...
        // Verify password
        const validPassword = await bcryptjs_1.default.compare(password, user.user_password);
        if (!validPassword) {
            res.status(401).json({
                error: true,
                message: "Invalid email or password",
        ...
        if (!user.is_verified) {
            await database_1.default.query("UPDATE users SET is_verified = 1, verified_at = NOW(), activation_code = NULL WHERE user_id = $1", [user.user_id]);
        }
...
const forgetPassword = async (req, res) => {
    try {
        ...
        const usersResult = await database_1.default.query("SELECT user_id, user_firstname, user_lastname, business_name, account_type FROM users WHERE user_email = $1 AND account_status = 'approved'", [email]);
        if (usersResult.rows.length === 0) {
        ...
        await database_1.default.query("UPDATE users SET reset_code = $1, reset_expires = $2 WHERE user_id = $3", [resetCode, resetExpires, user.user_id]);
        const recipientName = user.account_type === "business"
            ? user.business_name
            : `${user.user_firstname} ${user.user_lastname}`;
        // Send reset email
        await (0, emailConfig_1.sendEmail)(email, "Password Reset Code", `
...
const forgetPasswordConfirm = async (req, res) => {
    try {
        ...
        const usersResult = await database_1.default.query("SELECT user_id FROM users WHERE user_email = $1 AND reset_code = $2 AND reset_expires > NOW()", [email, reset_key]);
        if (usersResult.rows.length === 0) {
            res.status(400).json({
                error: true,
...
exports.forgetPasswordConfirm = forgetPasswordConfirm;
/**
...
const forgetPasswordReset = async (req, res) => {
    try {
        ...
        const hashedPassword = await bcryptjs_1.default.hash(password, 12);
        await database_1.default.query("UPDATE users SET user_password = $1, reset_code = NULL, reset_expires = NULL WHERE user_id = $2", [hashedPassword, usersResult.rows[0].user_id]);
        res.json({
            success: true,
...
const resendActivation = async (req, res) => {
    try {
        ...
        const activationExpires = new Date(Date.now() + 48 * 60 * 60 * 1000);
        await database_1.default.query("UPDATE users SET activation_code = $1, activation_expires = $2 WHERE user_id = $3", [newActivationCode, activationExpires, user.user_id]);
        const recipientName = user.account_type === "business"
            ? user.business_name
            : `${user.user_firstname} ${user.user_lastname}`;
        // Send new activation code
        await (0, emailConfig_1.sendEmail)(email, "New Activation Code", `
...
const adminLogin = async (req, res) => {
    try {
       ...
       FROM users WHERE user_email = $1`, [email]);
        ...
        await database_1.default.query("UPDATE users SET last_login = NOW(), user_last_seen = NOW() WHERE user_id = $1", [user.user_id]);
        // Check if JWT_SECRET is configured
        if (!process.env.JWT_SECRET) {
            console.error("JWT_SECRET is not configured in environment variables");
            res.status(500).json({
 ...
 */
const personalRegister = async (req, res) => {
    // Set account_type to 'personal' and call signUp
    req.body.account_type = "personal";
    await (0, exports.signUp)(req, res);
};
exports.personalRegister = personalRegister;
/**
